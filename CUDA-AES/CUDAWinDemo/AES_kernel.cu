...
#define DECRY_ROUND(t1, t2, rkeyoffset)	    t2[tiddiv4 + tidmix4] =  s_D[t1[index0] & 0xFF]\
						^ s_D[256 + ((t1[index1] >>  8) & 0xFF)]\
						^ s_D[512 + ((t1[index2] >> 16) & 0xFF)]\
						^ s_D[768 + ((t1[index3] >> 24) & 0xFF)]\
						^ s_rkey[(rkeyoffset) + tidmix4];

#define FINAL_DECRY_ROUND(t1, t2)	t2[tid] = (s_S[t1[tiddiv4 + ((tid - 0) & 3)] & 0xFF]\
					| (s_S[((t1[tiddiv4 + ((tid - 1) & 3)] >> 8) & 0xFF)  ] << 8)\
					| (s_S[((t1[tiddiv4 + ((tid - 2) & 3)] >> 16) & 0xFF) ] << 16)\
					| (s_S[((t1[tiddiv4 + ((tid - 3) & 3)] >> 24) & 0xFF) ] << 24))\
					^ s_rkey[tidmod4];

#define TEN_DECRY_ROUNDS		DECRY_ROUND( temp0, temp1, 36);\
						DECRY_ROUND( temp1, temp0, 32);\
						DECRY_ROUND( temp0, temp1, 28);\
						DECRY_ROUND( temp1, temp0, 24);\
						DECRY_ROUND( temp0, temp1, 20);\
						DECRY_ROUND( temp1, temp0, 16);\
						DECRY_ROUND( temp0, temp1, 12);\
						DECRY_ROUND( temp1, temp0,  8);\
						DECRY_ROUND( temp0, temp1,  4);\
						FINAL_DECRY_ROUND(temp1, temp0);


#define ENCRY_ROUND(t1, t2, rkeyoffset)	    t2[tiddiv4 + tidmix4] =  s_T[t1[index0] & 0xFF]\
						^ s_T[256 + ((t1[index1] >>  8) & 0xFF)]\
						^ s_T[512 + ((t1[index2] >> 16) & 0xFF)]\
						^ s_T[768 + ((t1[index3] >> 24) & 0xFF)]\
						^ s_rkey[(rkeyoffset) + tidmix4];

#define FINAL_ENCRY_ROUND(t1, t2)	t2[tid] = (s_S[t1[tiddiv4 + ((tid + 0) & 3)] & 0xFF]\
					| (s_S[((t1[tiddiv4 + ((tid + 1) & 3)] >> 8) & 0xFF)  ] << 8)\
					| (s_S[((t1[tiddiv4 + ((tid + 2) & 3)] >> 16) & 0xFF) ] << 16)\
					| (s_S[((t1[tiddiv4 + ((tid + 3) & 3)] >> 24) & 0xFF) ] << 24))\
					^ s_rkey[40 + tidmod4];

#define TEN_ENCRY_ROUNDS		ENCRY_ROUND( temp0, temp1, 4);\
						ENCRY_ROUND( temp1, temp0, 8);\
						ENCRY_ROUND( temp0, temp1, 12);\
						ENCRY_ROUND( temp1, temp0, 16);\
						ENCRY_ROUND( temp0, temp1, 20);\
						ENCRY_ROUND( temp1, temp0, 24);\
						ENCRY_ROUND( temp0, temp1, 28);\
						ENCRY_ROUND( temp1, temp0, 32);\
						ENCRY_ROUND( temp0, temp1, 36);\
						FINAL_ENCRY_ROUND(temp1, temp0);

//*********************************************************//
//     src		从global memory输入密文
//     dest		向global memory输出的解密结果
//     roundkey	加密使用的轮密钥
//********************************************************//
extern "C"  __global__ void
AES128_EBC_decry_kernel( const unsigned int* src, unsigned int* dest, unsigned int* roundkey) 
{
    //线程id
  ...
  // shared memory
  __shared__ unsigned int temp0[BLOCK_SIZE];	//中间变量temp0
  __shared__ unsigned int temp1[BLOCK_SIZE];	//中间变量temp1
  unsigned int index0;				//查找表索引index0
  unsigned int index1;				//查找表索引index1
  unsigned int index2;				//查找表索引index2
  unsigned int index3;				//查找表索引index3
  __shared__ unsigned int s_D[1024];			//D查找表
  __shared__ unsigned int s_S[256];				//逆S-box
  __shared__ unsigned int s_rkey[44];			//轮密钥

  // 从global memory 读入数据
  if(tid < 44)										//将轮密钥输入shared memory
  s_rkey[tid] = roundkey[tid];							

  s_D[tid] = D[tid];								//将D表输入shared memory
  s_D[tid + 256] = D[tid + 256];
  s_D[tid + 512] = D[tid + 512];
  s_D[tid + 768] = D[tid + 768];
  s_S[tid] = InvS[tid];								//将逆Sbox输入shared memory
  __syncthreads();

   //计算查表索引
   unsigned int tiddiv4 = ((tid >> 2) << 2);		//通过位运算加速tiddiv4 = (tid / 4) * 4, 四个线程共同处理block中第tid/4个aes分组
   unsigned int tidmod4 = tid - tiddiv4;			//tivmod4 = tiv%4

   unsigned int tidmix4 = tidmod4 == 0 ? 0 : tidmod4 == 1 ? 2 : tidmod4 == 2 ? 3 : 1;	//

   index0 = tiddiv4 +((tidmix4) & 3);			//逆行变换用索引
   index1 = tiddiv4 +((tidmix4 - 1) & 3);		
   index2 = tiddiv4 +((tidmix4 - 2) & 3);		
   index3 = tiddiv4 +((tidmix4 - 3) & 3);		

   for(unsigned int i = 0; i < LOOP_IN_BLOCK; i++)
   {
	   temp0[tid] = src[ bid * BLOCK_SIZE * LOOP_IN_BLOCK + BLOCK_SIZE * i + tid]; //将密文丛global memory读入

	   temp0[tid] ^= s_rkey[40 + tidmod4];//轮密钥加

	   TEN_DECRY_ROUNDS;	//十轮运算
	   
	   __syncthreads();
	   dest[ bid * BLOCK_SIZE * LOOP_IN_BLOCK + BLOCK_SIZE * i  + tid] = temp0[tid];// 将结果写回到global memory
   }
}

//*********************************************************//
//! Simple test kernel for device functionality
//! @param src		从global memory输入密文
//! @param dest		向global memory输出的解密结果
//! @param roundkey	解密使用的轮密钥
//*********************************************************//
extern "C"  __global__ void
AES128_EBC_encry_kernel( const unsigned int* src, unsigned int* dest, unsigned int* roundkey) 
{
//	access thread id
	const unsigned int tid  = threadIdx.x;
	const unsigned int bid  = blockIdx.x;
  ...
  unsigned int index2;				//查找表索引index2
  unsigned int index3;				//查找表索引index3
  __shared__ unsigned int s_T[1024];			//D查找表
  __shared__ unsigned int s_S[256];				//逆S-box
  __shared__ unsigned int s_rkey[44];			//轮密钥
  ...
  s_T[tid] = T[tid];								//将T表输入shared memory
  s_T[tid + 256] = T[tid + 256];
  s_T[tid + 512] = T[tid + 512];
  s_T[tid + 768] = T[tid + 768];
  s_S[tid] = Sbox[tid];								//将Sbox输入shared memory
  ...
  __syncthreads();
   ...
   unsigned int tidmix4 = tidmod4 == 0 ? 2 : tidmod4 == 1 ? 0 : tidmod4 == 2 ? 3 : 1;	//

   index0 = tiddiv4 +((tidmix4) & 3);			//行变换用索引
   index1 = tiddiv4 +((tidmix4 + 1) & 3);		//
   index2 = tiddiv4 +((tidmix4 + 2) & 3);		//
   index3 = tiddiv4 +((tidmix4 + 3) & 3);		//
   ...
   for(unsigned int i = 0; i < LOOP_IN_BLOCK; i++)
   {
	   ...
	   temp0[tid] ^= s_rkey[tidmod4];//轮密钥加

	   TEN_ENCRY_ROUNDS;	//十轮运算
...
extern "C" 
void AesSetKeyEncode(unsigned int * roundkey, const unsigned char *key, unsigned keySize)
{
	...
	for (i = 4; i < wSize; i++) 
	{
		unsigned int t = w[i - 1];
		unsigned rem = i % keySize;

		if (rem == 0) 
		{t = Ui32(h_Sbox[gb1(t)] ^ Rcon[i / keySize], h_Sbox[gb2(t)], h_Sbox[gb3(t)], h_Sbox[gb0(t)]);
		}
...
extern "C" 
void AesSetKeyDecode(unsigned int * roundkey, const unsigned char *key, unsigned keySize)
{
	unsigned int i, num;
	unsigned int *w;
	AesSetKeyEncode(roundkey, key, 16);
	num = 36;
	w = roundkey + 4;
	for (i = 0; i < num; i++)
	{
		...
		w[i] = 
			h_D[        h_Sbox[gb0(r)]] ^
			h_D[0x100 + h_Sbox[gb1(r)]] ^
			h_D[0x200 + h_Sbox[gb2(r)]] ^
			h_D[0x300 + h_Sbox[gb3(r)]];
	}
}
